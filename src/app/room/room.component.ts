...
export class RoomComponent implements OnInit {
  ...
  fetchHalls() {
    this.authService.getAllHalls().subscribe(
      (Halls: Room[]) => {
        this.Halls = Halls;
      },
      (error: any) => {
  ...
  onSubmit(): void {
    if (this.roomForm.valid) {
      ...
      formData.append('hallType', this.roomForm.get('hallType')?.value || '');
      ...
      this.authService.addRoom(formData).subscribe(
        response => {
          ...
          this.roomForm.reset();
          this.fetchHalls(); // Refresh room list
        },
        error => {
          console.error('Error Adding Room', error);
          alert(error.message || 'Error! Can’t Add Halls');
        }
      );
    }
  }
  ...
  onEditSubmit(): void {
    if (this.editRoomForm.valid && this.currentRoomId !== null) {
      ...
      const formData: FormData = new FormData();
      formData.append('hallType', this.editRoomForm.get('hallType')?.value || '');
